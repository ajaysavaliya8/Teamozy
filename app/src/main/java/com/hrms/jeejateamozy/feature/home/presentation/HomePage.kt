...
fun HomePage(
    ...
    // Timer management — recalculate from server time on every refresh
    LaunchedEffect(ui.currentState, ui.lastCheckInTime) {
        if (ui.currentState == "CHECK_OUT_NEEDED") {
            elapsedSeconds = calculateElapsedSeconds(ui.lastCheckInTime)
            isTimerRunning = true
        } else {
